// VoidPiano — Main application script (optimized for low latency)
...
const PIANO_PRESETS = [
  {
    id: 'salamander', name: 'SALAMANDER', desc: 'Yamaha C5 · Warm', emoji: '🎹',
  ...
  {
    id: 'bright', name: 'BRIGHT GRAND', desc: 'FluidR3 · Bright', emoji: '✨',
  ...
  {
    id: 'electric', name: 'ELECTRIC GRAND', desc: 'FluidR3 · Electric', emoji: '⚡',
  ...
  {
    id: 'honky', name: 'HONKY TONK', desc: 'FluidR3 · Vintage', emoji: '🎪',
...
// Build MIDI note name table once — avoids Tone.Frequency() on every keypress
const MIDI_NOTE_NAME = {};
(function() {
  const names = ['C','C#','D','D#','E','F','F#','G','G#','A','A#','B'];
  for (let i = 0; i < 128; i++) {
    const oct = Math.floor(i / 12) - 1;
    MIDI_NOTE_NAME[i] = names[i % 12] + oct;
  }
})();
...
// Cached piano canvas rect — updated on resize, avoids getBoundingClientRect() on every note
let cachedPianoRect = null;

// Pre-computed piano key hit areas
let whiteKeyHits = []; // [{midi, x, x2}]
let blackKeyHits = []; // [{midi, bx, bx2}]
...
async function initAudioAfterGesture() {
  try {
    ...
    setLoadProgress(40, 'LOADING GRAND PIANO...');
...
async function loadPianoSampler(preset) {
  return new Promise(function(resolve) {
    const newPiano = new Tone.Sampler({
      ...
      onload: function() { piano = newPiano; resolve(); },
    }).connect(pianoFilter);
    setTimeout(function() {
...
async function switchPiano(presetId) {
  ...
  const preset = PIANO_PRESETS.find(function(p) { return p.id === presetId; });
  if (!preset || !audioReady || !audioContextStarted) return;

  isLoadingPiano = true;
  pressedKeys.forEach(function(midi) {
    try {
      if (piano) piano.triggerRelease(MIDI_NOTE_NAME[midi], Tone.now());
    } catch(e) {}
  });
  pressedKeys.clear();
  renderPiano();
  ...
  } catch(e) { console.warn('Piano switch error:', e); }
...
function setPianoSelectorState(activeId, loading) {
  const preset = PIANO_PRESETS.find(function(p) { return p.id === activeId; });
...
// ===== PLAY / STOP NOTE (optimized — audio first, visuals after) =====
function playNote(midi, vel) {
  vel = vel !== undefined ? vel : 0.75;
  if (!audioReady || !piano || !audioContextStarted || isLoadingPiano) return;

  // 🔑 AUDIO FIRST — minimum latency
  const now = Tone.now();
  try { piano.triggerAttack(MIDI_NOTE_NAME[midi], now, vel); } catch(e) {}
...
function stopNote(midi) {
  ...
  // 🔑 AUDIO FIRST
  const now = Tone.now();
  try { piano.triggerRelease(MIDI_NOTE_NAME[midi], now + 0.05); } catch(e) {}
...
async function initMIDI() {
  ...
  try {
    ...
    function connect() {
      let found = false;
      acc.inputs.forEach(function(inp) { inp.onmidimessage = onMIDI; found = true; });
...
// MIDI handler — ultra lean, no extra work
function onMIDI(ev) {
...
// Pre-compute hit areas once — avoids recalculating on every mouse event
function buildKeyHitAreas() {
  whiteKeyHits = [];
  blackKeyHits = [];
  let wi = 0, wx = 0;
  pianoKeys.forEach(function(k) {
    if (!k.isBlack) {
      whiteKeyHits.push({ midi: k.midi, x: wi * WKW, x2: (wi + 1) * WKW });
      wx = wi * WKW;
      wi++;
    } else {
      const bx = wx + WKW - BKW / 2;
      blackKeyHits.push({ midi: k.midi, bx: bx, bx2: bx + BKW });
    }
  });
}

// Piano canvas — set size ONCE, use clearRect after
let pianoCvInitialized = false;

function renderPiano() {
  const cv = document.getElementById('pianoCanvas');
  const ctx = cv.getContext('2d');
  const whites = pianoKeys.filter(function(k) { return !k.isBlack; });

  // Set canvas size only once to avoid layout reflow on every note
  if (!pianoCvInitialized) {
    cv.width = whites.length * WKW;
    cv.height = WKH + 8;
    pianoCvInitialized = true;
  }
...
// Optimized hit test using pre-computed arrays
function pianoNoteAt(x, y) {
  // Black keys first (on top)
  for (let i = 0; i < blackKeyHits.length; i++) {
    const k = blackKeyHits[i];
    if (x >= k.bx && x <= k.bx2 && y <= BKH) return k.midi;
  }
  // White keys
  for (let i = 0; i < whiteKeyHits.length; i++) {
    const k = whiteKeyHits[i];
    if (x >= k.x && x <= k.x2) return k.midi;
  }
  return null;
}

// Update cached piano rect — called on resize and after init
function updatePianoRect() {
  const cv = document.getElementById('pianoCanvas');
  cachedPianoRect = cv.getBoundingClientRect();
}
...
function exportMIDI() {
  ...
  a.href = url; a.download = 'voidpiano_session.mid'; a.click();
...
function setupKnob(canvasId, getVal, setVal, onUpdate) {
  ...
  cv.addEventListener('dblclick', function(e) {
    e.preventDefault();
    const def = KNOB_DEFAULTS[canvasId];
    if (def !== undefined) { setVal(def); onUpdate(); }
  });
}
...
function resizeAnim() {
  ...
  setTimeout(updatePianoRect, 100);
}

function spawnParticles(midi) {
  // Use cached rect — no getBoundingClientRect() on hot path
  if (!cachedPianoRect) return;
  const ni = midi % 12;
  ...
  const xBase = cachedPianoRect.left + wi * WKW + WKW / 2;
  const yBase = cachedPianoRect.top;
  ...
  particles.push({
    ...
    shape: 'ripple', rot: 0, rotS: 0, life: 1, decay: 0.022,
  });
}
...
function animLoop() {
  ...
  // Iterate in reverse to splice dead particles without index issues
...
// Piano mouse events — use cached rect
const pianoCv = document.getElementById('pianoCanvas');
let mouseNote = null;

pianoCv.addEventListener('mousedown', function(e) {
  e.preventDefault();
  if (!cachedPianoRect) updatePianoRect();
  const n = pianoNoteAt(e.clientX - cachedPianoRect.left, e.clientY - cachedPianoRect.top);
...
pianoCv.addEventListener('touchstart', function(e) {
  ...
  Array.from(e.changedTouches).forEach(function(t) {
    const n = pianoNoteAt(t.clientX - cachedPianoRect.left, t.clientY - cachedPianoRect.top);
...
async function init() {
  ...
  renderPiano();
  renderAllKnobs();
  // Cache piano rect after first render
  setTimeout(updatePianoRect, 50);
  animLoop();
